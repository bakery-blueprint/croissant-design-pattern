package com.github.bakerybluprint.croissant.week_04.dy.homework.Order;
...
public abstract class OrderCheck {
    ...
    public final Map<String, Object> orderSheetRequest(Product product) {
        ...
        if (product.getGiftNo() > 0 && !checkGift(product.getGiftNo())) {
            result.put("STATUS", "E");
    ...
    private boolean checkProduct(Product product) {
        if (product.getStock() > 0) {
            LOGGER.info(product.getPrdCd() + "상품 재고 체크 : " + product.getStock());
            return true;
        } else {
            LOGGER.info(product.getPrdCd() + "상품 재고 없음");
            return false;
        }
    }
