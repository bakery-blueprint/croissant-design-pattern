package com.github.bakerybluprint.croissant.week_01.sh.order;
...
//        return false;
    ...
    public Map<String, String> orderSheetRequest(Customer customer, Product product) {
        ...
        //1.로그인 상태 체크
        if (!checkLoginStatus()) {
            preOrderStatus.put("S","로그인한 고객이 아닙니다.");
            return preOrderStatus;
        }

        //2.고객의 주문가능 여부 판단
        if (!checkCustomerStatus(customer)) {
            preOrderStatus.put("S","주문 가능한 고객이 아닙니다.");
            return preOrderStatus;
        }

        //3.주문 상품 재고 체크
        if (!checkProduct(product)) {
           preOrderStatus.put("S","주문 상품의 재고가 부족합니다.");
           return preOrderStatus;
        }

        //4.사은품 재고 체크
        if (!checkGift(String.valueOf(product.getGiftNo()))) {
            preOrderStatus.put("S","사은품 재고가 부족합니다.");
            return preOrderStatus;
        }

        preOrderStatus.put("E", "주문 가능");
        return preOrderStatus;
    }
}
